...
public class SamplingIVF {
  ...
  // cache these for now since our only real operation is to get the trained vector distances on
  // search
  ...
  public void train(RandomAccessVectorValues.Floats vectors, int dimensions) throws IOException {
    ...
    List<Integer> randomStartVectors =
        getRandomAndRemove(
    ...
    while (iterations < maxIterations) {
      ...
      for (int i = 0; i < centroids.length; i++) {
        ...
        // FIXME: FUTURE - this produces a potential set of NaN vectors when no vectors are near the
        // centroid; exclude those centroids?
